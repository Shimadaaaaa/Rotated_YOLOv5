...
class Model(nn.Module):
    ...
    def _initialize_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        ...
        for mi, s in zip(m.m, m.stride):  # from
            ...
            b[:, 8] += math.log(8 / (640 / s) ** 2)  # obj (8 objects per 640 image)
            b[:, 9:] += math.log(0.6 / (m.nc - 0.99)) if cf is None else torch.log(cf / cf.sum())  # cls
    ...
    def _print_biases(self):
        ...
        for mi in m.m:  # from
            ...
            print(('%6g Conv2d.bias:' + '%10.3g' * 10) % (mi.weight.shape[1], *b[:9].mean(1).tolist(), b[9:].mean()))
